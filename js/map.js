( function ( $, document, window, google, Bestwebsite, i18n ) {
	'use strict';
	...
	MapField.prototype = {
		...
		initDomElements: function () {
			this.$canvas = this.$container.find( '.Bestwebsite-map-canvas' );
			this.canvas = this.$canvas[0];
			this.$coordinate = this.$container.find( '.Bestwebsite-map-coordinate' );
		...
		addListeners: function () {
			...
			$( window ).on( 'Bestwebsite_map_refresh', refresh );

			// Refresh on meta box hide and show
			Bestwebsite.$document.on( 'postbox-toggled', refresh );
		...
		autocomplete: function () {
			...
			if ( $( '.Bestwebsite-geo-binding' ).length ) {
				var geocodeAddress = that.geocodeAddress.bind( that );
		...
		findAddressField: function( fieldName ) {
			...
			$address = this.$container.closest( '.Bestwebsite-group-clone' ).find( 'input[name*="[' + fieldName + ']"]' );
			if ( $address.length ) {
				return $address;
			}

			// If map and address is inside a non-cloneable group.
			$address = this.$container.closest( '.Bestwebsite-group-wrapper' ).find( 'input[name*="[' + fieldName + ']"]' );
			if ( $address.length ) {
				return $address;
			}
	...
	function init( e ) {
		$( e.target ).find( '.Bestwebsite-map-field' ).each( createController );
	}

	function restart() {
		$( '.Bestwebsite-map-field' ).each( createController );
	}

	Bestwebsite.$document
		.on( 'mb_ready', init )
		.on( 'clone', '.Bestwebsite-input', restart );
} )( jQuery, document, window, google, Bestwebsite, Bestwebsite_Map );
