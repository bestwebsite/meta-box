( function ( $, wp, Bestwebsite ) {
	...
	UploadButton = views.UploadButton = Backbone.View.extend( {
		className: 'Bestwebsite-upload-area',
		tagName: 'div',
		template: wp.template( 'Bestwebsite-upload-area' ),
		render: function () {
		...
		initialize: function ( options ) {
			...
			this.el.id = _.uniqueId( 'Bestwebsite-upload-area-' );
			this.render();
		...
		initUploader: function () {
			var self = this,
				...
				options = {
					container: this.el,
					dropzone: this.el,
					browser: this.$( '.Bestwebsite-browse-button' ),
					params: {
						post_id : $( '#post_ID' ).val()
					},
			...
			if ( extensions ) {
				filters.mime_types = [{title: i18nBestwebsiteMedia.select, extensions: extensions}];
			}
		...
		getExtensions: function () {
			...
			_.each( mimeTypes, function ( current, index ) {
				if ( i18nBestwebsiteMedia.extensions[current] ) {
					exts = exts.concat( i18nBestwebsiteMedia.extensions[current] );
				}
			} );
			return exts;
		}
	...
	function init( e ) {
		$( e.target ).find( '.Bestwebsite-file_upload' ).each( initFileUpload );
	}

	Bestwebsite.$document
		.on( 'mb_ready', init )
		.on( 'clone', '.Bestwebsite-file_upload', initFileUpload )
} )( jQuery, wp, Bestwebsite );
