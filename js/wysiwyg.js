( function ( $, wp, window, Bestwebsite ) {
	'use strict';
	...
	function getOriginalId( $el ) {
		var $clone = $el.closest( '.Bestwebsite-clone' ),
			currentId = $clone.find( '.Bestwebsite-wysiwyg' ).attr( 'id' );
	...
	function init( e ) {
		$( e.target ).find( '.Bestwebsite-wysiwyg' ).each( transform );
	}
	...
	Bestwebsite.$document
		...
		.on( 'clone', '.Bestwebsite-wysiwyg', function() {
			/*
