( function ( $, Bestwebsite ) {
	...
	function toggleTree() {
		var $this = $( this ),
			val = $this.val(),
			$tree = $this.siblings( '.Bestwebsite-select-tree' ),
	...
	function init( e ) {
		...
		$el.find( '.Bestwebsite-select-tree > select' ).select2();
		$el.find( '.Bestwebsite-select-tree > select' ).each( setInitialRequiredProp );
	}

	Bestwebsite.$document
		.on( 'mb_ready', init )
		.on( 'change', '.Bestwebsite-select-tree > select', toggleTree )
		.on( 'clone', '.Bestwebsite-select-tree > select', instantiateSelect2 );
} )( jQuery, Bestwebsite );
