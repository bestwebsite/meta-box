( function ( $, Bestwebsite ) {
	...
	function transform() {
		...
		if ( options.ajax_data ) {
			...
			options.ajax.transport = function ( params, success, failure ) {
				...
				var actions = {
					'post'             : 'Bestwebsite_get_posts',
					'taxonomy'         : 'Bestwebsite_get_terms',
					'taxonomy_advanced': 'Bestwebsite_get_terms',
					'user'             : 'Bestwebsite_get_users'
				};
	...
	function init( e ) {
		$( e.target ).find( '.Bestwebsite-select_advanced' ).each( transform );
	}

	Bestwebsite.$document
		.on( 'mb_ready', init )
		.on( 'clone', '.Bestwebsite-select_advanced', transform );
} )( jQuery, Bestwebsite );
