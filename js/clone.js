( function ( $, Bestwebsite ) {
	...
	var cloneIndex = {
		/**
		 * Set index for fields in a .Bestwebsite-clone
		 * @param $inputs .Bestwebsite-clone element
		...
		set: function ( $inputs, index ) {
			$inputs.each( function () {
				...
				// Name attribute
				var name = this.name;
				if ( name && ! $field.closest( '.Bestwebsite-group-clone' ).length ) {
		 ...
		 * @param $container .Bestwebsite-input container
		 * @return integer
		 */
	...
	var cloneValue = {
		setDefault: function() {
			...
			} else if ( $field.hasClass( 'Bestwebsite-checkbox' ) ) {
					$field.prop( 'checked', !! defaultValue );
			} else if ( $field.hasClass( 'Bestwebsite-checkbox_list' ) ) {
				var value = $field.val();
			...
			} else if ( ! $field.hasClass( 'Bestwebsite-hidden' ) ) {
	...
	function clone( $container ) {
		var $last = $container.children( '.Bestwebsite-clone' ).last(),
			$clone = $last.clone(),
			nextIndex = cloneIndex.nextIndex( $container );

		// Clear fields' values.
		var $inputs = $clone.find( Bestwebsite.inputSelectors );
	...
	function toggleRemoveButtons( $container ) {
		var $clones = $container.children( '.Bestwebsite-clone' );
		$clones.children( '.remove-clone' ).toggle( $clones.length > 1 );

		// Recursive for nested groups.
		$container.find( '.Bestwebsite-input' ).each( function () {
	...
	function toggleAddButton( $container ) {
		var $button = $container.children( '.add-clone' ),
			...
			numClone = $container.children( '.Bestwebsite-clone' ).length;
	...
	function addClone( e ) {
		...
		var $container = $( this ).closest( '.Bestwebsite-input' );
		clone( $container );
	...
	function removeClone( e ) {
		...
		var $this = $( this ),
			$container = $this.closest( '.Bestwebsite-input' );

		// Remove clone only if there are 2 or more of them
		if ( $container.children( '.Bestwebsite-clone' ).length < 2 ) {
			return;
		}
		...
		// Trigger custom change event for MB Blocks to update block attributes.
		$container.find( Bestwebsite.inputSelectors ).first().trigger( 'mb_change' );
	}

	/**
	 * Sort clones.
	 * Expect this = .Bestwebsite-input element.
	 */
	function sortClones() {
		var $container = $( this );
		...
		if ( 0 === $container.children( '.Bestwebsite-clone' ).length ) {
			return;
		}

		$container.sortable( {
			handle: '.Bestwebsite-clone-icon',
			placeholder: ' Bestwebsite-clone Bestwebsite-sortable-placeholder',
			items: '> .Bestwebsite-clone',
			start: function ( event, ui ) {
				...
				ui.item.find( '.Bestwebsite-wysiwyg' ).each( function () {
	...
	function start() {
		...
		$container.data( 'next-index', $container.children( '.Bestwebsite-clone' ).length );
		sortClones.apply( this );
	}

	function init( e ) {
		$( e.target ).find( '.Bestwebsite-input' ).each( start );
	}
