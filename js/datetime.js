( function ( $, _, Bestwebsite, i18n ) {
	...
	function transform() {
		var $this = $( this ),
			...
			$timestamp = $this.siblings( '.Bestwebsite-datetime-timestamp' ),
			current = $this.val(),
	...
	function init( e ) {
		$( e.target ).find( '.Bestwebsite-datetime' ).each( transform );
	}

	setTimeI18n();
	Bestwebsite.$document
		.on( 'mb_ready', init )
		.on( 'clone', '.Bestwebsite-datetime', transform );
} )( jQuery, _, Bestwebsite, Bestwebsite_Datetime );
