( function ( $, wp, _, Bestwebsite, i18n ) {
	'use strict';

	var views = Bestwebsite.views = Bestwebsite.views || {},
		models = Bestwebsite.models = Bestwebsite.models || {},
		media = wp.media,
	...
	MediaCollection = Backbone.Collection.extend( {
		...
		remove: function ( models, options ) {
			...
			if( ! $( event.target ).closest( '.Bestwebsite-field, [data-class="Bestwebsite-field"]' ).length ) {
				return;
			}
	...
	MediaField = views.MediaField = Backbone.View.extend( {
		className: 'Bestwebsite-media-view',
		initialize: function ( options ) {
			...
			this.controller.get( 'items' ).on( 'add remove reset', _.debounce( function () {
				that.$input.trigger( 'change', [that.$( '.Bestwebsite-media-input' )] );
			}, 500 ) );
	...
	MediaList = views.MediaList = Backbone.View.extend( {
		tagName: 'ul',
		className: 'Bestwebsite-media-list',
	...
	MediaStatus = views.MediaStatus = Backbone.View.extend( {
		tagName: 'div',
		className: 'Bestwebsite-media-status',
		template: wp.template( 'Bestwebsite-media-status' ),
	...
	MediaButton = views.MediaButton = Backbone.View.extend( {
		tagName: 'div',
		className: 'Bestwebsite-media-add',
		template: wp.template( 'Bestwebsite-media-button' ),
		events: {
	...
	MediaItem = views.MediaItem = Backbone.View.extend( {
		tagName: 'li',
		className: 'Bestwebsite-media-item attachment',
		template: wp.template( 'Bestwebsite-media-item' ),
		initialize: function ( options ) {
			this.controller = options.controller;
			this.collection = this.controller.get( 'items' );
			this.render();
		...
		events: {
			'click .Bestwebsite-image-overlay': function ( e ) {
				e.preventDefault();
				this.trigger( 'click:switch', this.model );
			},
			'click .Bestwebsite-remove-media': function ( e ) {
				e.preventDefault();
				this.trigger( 'click:remove', this.model );
			},
			'click .Bestwebsite-edit-media': function ( e ) {
				e.preventDefault();
	...
	MediaSelect = views.MediaSelect = MediaFrame.Select.extend( {
		...
		createStates: function () {
			...
			this.$el.attr( 'data-class', 'Bestwebsite-field' );
	...
	function init( e ) {
		$( e.target ).find( '.Bestwebsite-file_advanced' ).each( initMediaField );
	}

	Bestwebsite.$document
		.on( 'mb_ready', init )
		.on( 'clone', '.Bestwebsite-file_advanced', initMediaField );
} )( jQuery, wp, _, Bestwebsite, i18nBestwebsiteMedia );
