( function( $, L, Bestwebsite, i18n ) {
	...
	OsmField.prototype = {
		...
		initDomElements: function () {
			this.$canvas = this.$container.find( '.Bestwebsite-osm-canvas' );
			this.canvas = this.$canvas[0];
			this.$coordinate = this.$container.find( '.Bestwebsite-osm-coordinate' );
	...
	function init( e ) {
		$( e.target ).find( '.Bestwebsite-osm-field' ).each( createController );
	}

	function restart() {
		$( '.Bestwebsite-osm-field' ).each( createController );
	}

	Bestwebsite.$document
		.on( 'mb_ready', init )
		.on( 'clone', '.Bestwebsite-input', restart );
} )( jQuery, L, Bestwebsite, Bestwebsite_Osm );