...
class Bestwebsite_Media_Modal {
	...
	public function init() {
		...
		add_filter( 'Bestwebsite_show', array( $this, 'is_in_normal_mode' ), 10, 2 );
	}
	...
	public function get_fields() {
		$meta_boxes = Bestwebsite_get_registry( 'meta_box' )->all();
	...
	public function add_fields( $form_fields, WP_Post $post ) {
		foreach ( $this->fields as $field ) {
			...
			$meta                = Bestwebsite_Field::call( $field, 'meta', $post->ID, true );
			$form_field['value'] = $meta;

			$field['field_name'] = 'attachments[' . $post->ID . '][' . $field['field_name'] . ']';
			$form_field['html']  = Bestwebsite_Field::call( $field, 'html', $meta );
	...
	public function save_fields( $post, $attachment ) {
		foreach ( $this->fields as $field ) {
			...
			$old = Bestwebsite_Field::call( $field, 'raw_meta', $post['ID'] );
			$new = isset( $attachment[ $key ] ) ? $attachment[ $key ] : '';

			$new = Bestwebsite_Field::process_value( $new, $post['ID'], $field );

			// Call defined method to save meta value, if there's no methods, call common one.
			Bestwebsite_Field::call( $field, 'save', $new, $old, $post['ID'] );
		}
