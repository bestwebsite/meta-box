 ...
 * The field base class.
 */
abstract class Bestwebsite_Field {
	/**
	 * Add actions.
	 */
	...
	public static function show( $field, $saved, $post_id = 0 ) {
		...
		if ( $field['clone'] ) {
			$field_html = Bestwebsite_Clone::html( $meta, $field );
		} else {
		...
		$classes = "Bestwebsite-field Bestwebsite-{$field['type']}-wrapper " . $field['class'];
	...
	public static function begin_html( $meta, $field ) {
		...
		if ( $field['name'] ) {
			$field_label = sprintf(
				'<div class="Bestwebsite-label">
					<label for="%s">%s%s</label>
					%s
				</div>',
				esc_attr( $field['id'] ),
				$field['name'],
				$field['required'] || ! empty( $field['attributes']['required'] ) ? '<span class="Bestwebsite-required">*</span>' : '',
	...
	public static function end_html( $meta, $field ) {
		return Bestwebsite_Clone::add_clone_button( $field ) . self::call( 'input_description', $field ) . '</div>';
	}
	 ...
	 * @param array $field     Field parameters.
	 * @param array $args      Arguments of {@see Bestwebsite_meta()} helper.
	 *
	 * @return mixed
	 */
	public static function raw_meta( $object_id, $field, $args = array() ) {
		...
		if ( isset( $field['storage'] ) ) {
			...
			$storage = Bestwebsite_get_storage( $args['object_type'] );
		} else {
			$storage = Bestwebsite_get_storage( 'post' );
		}
	...
	public static function meta( $post_id, $saved, $field ) {
		...
		if ( $field['clone'] ) {
			$meta = Bestwebsite_Helpers_Array::ensure( $meta );
			...
			if ( $field['multiple'] ) {
				...
				$meta = is_array( $first ) ? array_map( 'Bestwebsite_Helpers_Array::ensure', $meta ) : array( $meta );
			}
	...
	public static function process_value( $value, $object_id, $field ) {
		...
		if ( $field['clone'] ) {
			$value = Bestwebsite_Clone::value( $value, $old_value, $object_id, $field );
		} else {
	...
	public static function save( $new, $old, $post_id, $field ) {
		...
		if ( ! Bestwebsite_Helpers_Value::is_valid_for_field( $new ) ) {
	...
	public static function get_attributes( $field, $value = null ) {
		...
		$attributes['class'] = trim( implode( ' ', array_merge( array( "Bestwebsite-{$field['type']}" ), (array) $attributes['class'] ) ) );
	...
	public static function render_attributes( $attributes ) {
		...
		$attributes = array_filter( $attributes, 'Bestwebsite_Helpers_Value::is_valid_for_attribute' );
	 ...
	 * Bestwebsite_the_field function later.
	 *
	 * @use self::get_value()
	 * @see Bestwebsite_the_value()
	 *
	 * @param  array    $field   Field parameters.
	 * @param  array    $args    Additional arguments. Rarely used. See specific fields for details.
	 * @param  int|null $post_id Post ID. null for current post. Optional.
	...
	public static function call() {
		...
		return call_user_func_array( array( Bestwebsite_Helpers_Field::get_class( $field ), $method ), $args );
	}
	 ...
	 * Filters:
	 * - Bestwebsite_{$name}
	 * - Bestwebsite_{$field['type']}_{$name}
	 * - Bestwebsite_{$field['id']}_{$name}
	 *
	 * @return mixed
	 */
	public static function filter() {
		...
		if ( $field['id'] ) {
			...
			$filters[] = 'Bestwebsite_' . $field_id . '_' . $name;
		}
