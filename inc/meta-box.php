...
class RW_Meta_Box {
	...
	public function __construct( $meta_box ) {
		...
		$this->meta_box = apply_filters( 'Bestwebsite_meta_box_settings', $this->meta_box );
	...
	public function register_fields() {
		$field_registry = Bestwebsite_get_registry( 'field' );
	...
	public function is_shown() {
		$show = apply_filters( 'Bestwebsite_show', true, $this->meta_box );

		return apply_filters( "Bestwebsite_show_{$this->id}", $show, $this->meta_box );
	}
	...
	protected function global_hooks() {
		...
		foreach ( $this->fields as $field ) {
			Bestwebsite_Field::call( $field, 'add_actions' );
		}
	}
	...
	public function enqueue() {
		...
		wp_enqueue_style( 'Bestwebsite', Bestwebsite_CSS_URL . 'style.css', array(), Bestwebsite_VER );
		if ( is_rtl() ) {
			wp_enqueue_style( 'Bestwebsite-rtl', Bestwebsite_CSS_URL . 'style-rtl.css', array(), Bestwebsite_VER );
		}

		wp_enqueue_script( 'Bestwebsite', Bestwebsite_JS_URL . 'script.js', array( 'jquery' ), Bestwebsite_VER, true );

		// Load clone script conditionally.
		foreach ( $this->fields as $field ) {
			if ( $field['clone'] ) {
				wp_enqueue_script( 'Bestwebsite-clone', Bestwebsite_JS_URL . 'clone.js', array( 'jquery-ui-sortable' ), Bestwebsite_VER, true );
				break;
			}
		}
		...
		foreach ( $this->fields as $field ) {
			Bestwebsite_Field::call( $field, 'admin_enqueue_scripts' );
		}

		// Auto save.
		if ( $this->autosave ) {
			wp_enqueue_script( 'Bestwebsite-autosave', Bestwebsite_JS_URL . 'autosave.js', array( 'jquery' ), Bestwebsite_VER, true );
		}
		 ...
		 */
		do_action( 'Bestwebsite_enqueue_scripts', $this );
	}
	...
	public function postbox_classes( $classes ) {
		...
		$classes[] = "Bestwebsite-{$this->style}";
	...
	public function show() {
		...
		printf(
			...
			esc_attr( trim( "Bestwebsite-meta-box {$this->class}" ) ),
			esc_attr( $this->autosave ? 'true' : 'false' ),
			esc_attr( $this->object_type ),
			esc_attr( $this->object_id )
		);

		wp_nonce_field( "Bestwebsite-save-{$this->id}", "nonce_{$this->id}" );

		// Allow users to add custom code before meta box content.
		// 1st action applies to all meta boxes.
		// 2nd action applies to only current meta box.
		do_action( 'Bestwebsite_before', $this );
		do_action( "Bestwebsite_before_{$this->id}", $this );

		foreach ( $this->fields as $field ) {
			Bestwebsite_Field::call( 'show', $field, $saved, $this->object_id );
		}

		// Allow users to add custom code after meta box content.
		// 1st action applies to all meta boxes.
		// 2nd action applies to only current meta box.
		do_action( 'Bestwebsite_after', $this );
		do_action( "Bestwebsite_after_{$this->id}", $this );
	...
	public function save_post( $object_id ) {
		...
		// Before save action.
		do_action( 'Bestwebsite_before_save_post', $object_id );
		do_action( "Bestwebsite_{$this->id}_before_save_post", $object_id );

		array_map( array( $this, 'save_field' ), $this->fields );

		// After save action.
		do_action( 'Bestwebsite_after_save_post', $object_id );
		do_action( "Bestwebsite_{$this->id}_after_save_post", $object_id );
	}
	...
	public function save_field( $field ) {
		...
		$old     = Bestwebsite_Field::call( $field, 'raw_meta', $this->object_id );
		$new     = Bestwebsite_request()->post( $field['id'], $default );
		$new     = Bestwebsite_Field::process_value( $new, $this->object_id, $field );

		// Filter to allow the field to be modified.
		$field = Bestwebsite_Field::filter( 'field', $field, $field, $new, $old );

		// Call defined method to save meta value, if there's no methods, call common one.
		Bestwebsite_Field::call( $field, 'save', $new, $old, $this->object_id );

		Bestwebsite_Field::filter( 'after_save_field', null, $field, $new, $old, $this->object_id );
	}
	...
	public function validate() {
		$nonce = Bestwebsite_request()->filter_post( "nonce_{$this->id}", FILTER_SANITIZE_STRING );

		return ! $this->saved
			&& ( ! defined( 'DOING_AUTOSAVE' ) || $this->autosave )
			&& wp_verify_nonce( $nonce, "Bestwebsite-save-{$this->id}" );
	}
	...
	public static function normalize( $meta_box ) {
		 ...
		 * @since 4.4.1
		 */
		Bestwebsite_Helpers_Array::change_key( $meta_box, 'pages', 'post_types' );

		// Make sure the post type is an array and is sanitized.
		$meta_box['post_types'] = array_map( 'sanitize_key', Bestwebsite_Helpers_Array::from_csv( $meta_box['post_types'] ) );
	 ...
	 * @param Bestwebsite_Storage_Interface $storage Storage object. Optional.
	 *
	...
	public static function normalize_fields( $fields, $storage = null ) {
		foreach ( $fields as $k => $field ) {
			...
			$field = apply_filters( 'Bestwebsite_normalize_field', $field );
			$field = apply_filters( "Bestwebsite_normalize_{$field['type']}_field", $field );
			$field = apply_filters( "Bestwebsite_normalize_{$field['id']}_field", $field );
	...
	public function is_saved() {
		foreach ( $this->fields as $field ) {
			...
			$value = Bestwebsite_Field::call( $field, 'raw_meta', $this->object_id );
			if ( false === $value ) {
				continue;
			}
			if (
	...
	public function get_storage() {
		return Bestwebsite_get_storage( $this->object_type, $this );
	}
