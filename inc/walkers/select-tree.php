 ...
 */
class Bestwebsite_Walker_Select_Tree {
	/**
	 * Field settings.
	 *
	 * @var string
	...
	public function display_level( $options, $parent_id = 0, $active = false ) {
		$field      = $this->field;
		$walker     = new Bestwebsite_Walker_Select( $field, $this->meta );
		$attributes = Bestwebsite_Field::call( 'get_attributes', $field, $this->meta );

		$children = $options[ $parent_id ];
		$output   = sprintf(
			'<div class="Bestwebsite-select-tree %s" data-parent-id="%s"><select %s>',
			$active ? '' : 'hidden',
			esc_attr( $parent_id ),
