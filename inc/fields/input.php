 ...
 */
abstract class Bestwebsite_Input_Field extends Bestwebsite_Field {
	/**
	 * Enqueue scripts and styles.
	 */
	public static function admin_enqueue_scripts() {
		wp_enqueue_style( 'Bestwebsite-input', Bestwebsite_CSS_URL . 'input.css', '', Bestwebsite_VER );
	}
	...
	public static function html( $meta, $field ) {
		...
		$wrapper_class = 'Bestwebsite-input-group';
		if ( version_compare( get_bloginfo( 'version' ), '5.3', '>=' ) ) {
			$wrapper_class .= ' Bestwebsite-input-dark';
		}
		...
		if ( $field['prepend'] ) {
			$output .= '<span class="Bestwebsite-input-group-prepend">' . esc_html( $field['prepend'] ) . '</span>';
		}
		...
		if ( $field['append'] ) {
			$output .= '<span class="Bestwebsite-input-group-append">' . esc_html( $field['append'] ) . '</span>';
		}
