 ...
 * The Background field.
 */
class Bestwebsite_Background_Field extends Bestwebsite_Field {
	/**
	 * Enqueue scripts and styles.
	 */
	public static function admin_enqueue_scripts() {
		wp_enqueue_style( 'Bestwebsite-background', Bestwebsite_CSS_URL . 'background.css', '', Bestwebsite_VER );

		$args  = func_get_args();
		$field = reset( $args );
		$color = Bestwebsite_Color_Field::normalize(
			array(
				'type'          => 'color',
				'id'            => "{$field['id']}_color",
		...
		Bestwebsite_Color_Field::admin_enqueue_scripts( $color );
		Bestwebsite_File_Input_Field::admin_enqueue_scripts();
	}
	...
	public static function html( $meta, $field ) {
		...
		$output = '<div class="Bestwebsite-background-row">';
		...
		$output .= Bestwebsite_Color_Field::html( $meta['color'], $color );

		$output .= '</div><!-- .Bestwebsite-background-row -->';
		$output .= '<div class="Bestwebsite-background-row">';

		// Image.
		$image   = Bestwebsite_File_Input_Field::normalize(
			array(
				'type'        => 'file_input',
				'id'          => "{$field['id']}_image",
		...
		$output .= Bestwebsite_File_Input_Field::html( $meta['image'], $image );

		$output .= '</div><!-- .Bestwebsite-background-row -->';
		$output .= '<div class="Bestwebsite-background-row">';

		// Repeat.
		$repeat  = Bestwebsite_Select_Field::normalize(
			array(
				'type'        => 'select',
				'id'          => "{$field['id']}_repeat",
		...
		$output .= Bestwebsite_Select_Field::html( $meta['repeat'], $repeat );

		// Position.
		$position = Bestwebsite_Select_Field::normalize(
			array(
				'type'        => 'select',
		...
		$output  .= Bestwebsite_Select_Field::html( $meta['position'], $position );

		// Attachment.
		$attachment = Bestwebsite_Select_Field::normalize(
			array(
				'type'        => 'select',
		...
		$output    .= Bestwebsite_Select_Field::html( $meta['attachment'], $attachment );

		// Size.
		$size    = Bestwebsite_Select_Field::normalize(
			array(
				'type'        => 'select',
				'id'          => "{$field['id']}_size",
		...
		$output .= Bestwebsite_Select_Field::html( $meta['size'], $size );
		$output .= '</div><!-- .Bestwebsite-background-row -->';
