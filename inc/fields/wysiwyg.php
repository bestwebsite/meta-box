 ...
 */
class Bestwebsite_Wysiwyg_Field extends Bestwebsite_Field {
	/**
	...
	public static function admin_enqueue_scripts() {
		wp_enqueue_editor();
		wp_enqueue_style( 'Bestwebsite-wysiwyg', Bestwebsite_CSS_URL . 'wysiwyg.css', array(), Bestwebsite_VER );
		wp_enqueue_script( 'Bestwebsite-wysiwyg', Bestwebsite_JS_URL . 'wysiwyg.js', array( 'jquery' ), Bestwebsite_VER, true );
	}
	...
	public static function normalize( $field ) {
		...
		$field['options'] = wp_parse_args(
			$field['options'],
			array(
				'editor_class' => 'Bestwebsite-wysiwyg',
				'dfw'          => true, // Use default WordPress full screen UI.
			)
		);

		// Keep the filter to be compatible with previous versions.
		$field['options'] = apply_filters( 'Bestwebsite_wysiwyg_settings', $field['options'] );
