 ...
 */
class Bestwebsite_File_Field extends Bestwebsite_Field {
	/**
	 * Enqueue scripts and styles.
	 */
	public static function admin_enqueue_scripts() {
		wp_enqueue_style( 'Bestwebsite-file', Bestwebsite_CSS_URL . 'file.css', array(), Bestwebsite_VER );
		wp_enqueue_script( 'Bestwebsite-file', Bestwebsite_JS_URL . 'file.js', array( 'jquery-ui-sortable' ), Bestwebsite_VER, true );
	...
	public static function add_actions() {
		...
		add_action( 'wp_ajax_Bestwebsite_delete_file', array( __CLASS__, 'ajax_delete_file' ) );
	}
	...
	public static function ajax_delete_file() {
		$request = Bestwebsite_request();

		$field_id = $request->filter_post( 'field_id', FILTER_SANITIZE_STRING );
		check_ajax_referer( "Bestwebsite-delete-file_{$field_id}" );
		...
		$field       = Bestwebsite_get_field_settings( $field_id, array( 'object_type' => $object_type ), $object_id );
	...
	public static function html( $meta, $field ) {
		...
		$i18n_more = apply_filters( 'Bestwebsite_file_add_string', _x( '+ Add new file', 'file upload', 'meta-box' ), $field );
		...
		$attributes['class'] = 'Bestwebsite-file-input';
		...
		// Upload new files.
		$html .= sprintf(
			'<div class="Bestwebsite-file-new"><input %s>',
			self::render_attributes( $attributes )
		);
		if ( 1 !== $field['max_file_uploads'] ) {
			$html .= sprintf(
				'<a class="Bestwebsite-file-add" href="#"><strong>%s</strong></a>',
				$i18n_more
			);
		}
		$html .= '</div>';
		...
		$html .= sprintf(
			'<input type="hidden" class="Bestwebsite-file-index" name="%s" value="%s">',
			$field['index_name'],
			$field['input_name']
		);
	...
	protected static function get_uploaded_files( $files, $field ) {
		$reorder_nonce = wp_create_nonce( "Bestwebsite-reorder-files_{$field['id']}" );
		$delete_nonce  = wp_create_nonce( "Bestwebsite-delete-file_{$field['id']}" );
		$output        = '';
		...
		return sprintf(
			'<ul class="Bestwebsite-uploaded" data-field_id="%s" data-delete_nonce="%s" data-reorder_nonce="%s" data-force_delete="%s" data-max_file_uploads="%s" data-mime_type="%s">%s</ul>',
			$field['id'],
			$delete_nonce,
			$reorder_nonce,
	...
	protected static function file_html( $file, $index, $field ) {
		$i18n_delete = apply_filters( 'Bestwebsite_file_delete_string', _x( 'Delete', 'file upload', 'meta-box' ) );
		$i18n_edit   = apply_filters( 'Bestwebsite_file_edit_string', _x( 'Edit', 'file upload', 'meta-box' ) );
		...
		if ( $field['upload_dir'] ) {
			...
			if ( $edit_link ) {
				$data['edit_link'] = sprintf( '<a href="%s" class="Bestwebsite-file-edit" target="_blank"><span class="dashicons dashicons-edit"></span>%s</a>', $edit_link, $i18n_edit );
			}
		}

		return sprintf(
			'<li class="Bestwebsite-file">
				<div class="Bestwebsite-file-icon"><a href="%s" target="_blank">%s</a></div>
				<div class="Bestwebsite-file-info">
					<a href="%s" target="_blank" class="Bestwebsite-file-title">%s</a>
					<p class="Bestwebsite-file-name">%s</p>
					<p class="Bestwebsite-file-actions">
						%s
						<a href="#" class="Bestwebsite-file-delete" data-attachment_id="%s"><span class="dashicons dashicons-no-alt"></span>%s</a>
					</p>
				</div>
