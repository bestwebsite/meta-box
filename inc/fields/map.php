 ...
 * Map field class.
 */
class Bestwebsite_Map_Field extends Bestwebsite_Field {
	/**
	 * Enqueue scripts and styles.
	 */
	public static function admin_enqueue_scripts() {
		wp_enqueue_style( 'Bestwebsite-map', Bestwebsite_CSS_URL . 'map.css', array(), Bestwebsite_VER );
		 ...
		 */
		$google_maps_url = apply_filters( 'Bestwebsite_google_maps_url', $google_maps_url );
		wp_register_script( 'google-maps', esc_url_raw( $google_maps_url ), array(), Bestwebsite_VER, true );
		wp_enqueue_script(
			'Bestwebsite-map',
			Bestwebsite_JS_URL . 'map.js',
			array(
	...
	public static function html( $meta, $field ) {
		...
		$html    = sprintf(
			'<div class="Bestwebsite-map-field" data-address-field="%s">',
			esc_attr( $address )
		);

		$html .= sprintf(
			'<div class="Bestwebsite-map-canvas" data-default-loc="%s" data-region="%s"></div>
			<input type="hidden" name="%s" class="Bestwebsite-map-coordinate" value="%s">',
			esc_attr( $field['std'] ),
	...
	public static function render_map( $location, $args = array() ) {
		...
		$google_maps_url = apply_filters( 'Bestwebsite_google_maps_url', $google_maps_url );
		wp_register_script( 'google-maps', esc_url_raw( $google_maps_url ), array(), Bestwebsite_VER, true );
		wp_enqueue_script( 'Bestwebsite-map-frontend', Bestwebsite_JS_URL . 'map-frontend.js', array( 'google-maps', 'jquery' ), Bestwebsite_VER, true );
		...
		$output = sprintf(
			'<div class="Bestwebsite-map-canvas" data-map_options="%s" style="width:%s;height:%s"></div>',
