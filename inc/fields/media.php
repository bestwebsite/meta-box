 ...
 */
class Bestwebsite_Media_Field extends Bestwebsite_File_Field {
	/**
	...
	public static function admin_enqueue_scripts() {
		...
		wp_enqueue_style( 'Bestwebsite-media', Bestwebsite_CSS_URL . 'media.css', array(), Bestwebsite_VER );
		wp_enqueue_script( 'Bestwebsite-media', Bestwebsite_JS_URL . 'media.js', array( 'jquery-ui-sortable', 'underscore', 'backbone', 'media-grid' ), Bestwebsite_VER, true );

		Bestwebsite_Helpers_Field::localize_script_once(
			'Bestwebsite-media',
			'i18nBestwebsiteMedia',
			array(
				'add'                => apply_filters( 'Bestwebsite_media_add_string', _x( '+ Add Media', 'media', 'meta-box' ) ),
				'single'             => apply_filters( 'Bestwebsite_media_single_files_string', _x( ' file', 'media', 'meta-box' ) ),
				'multiple'           => apply_filters( 'Bestwebsite_media_multiple_files_string', _x( ' files', 'media', 'meta-box' ) ),
				'remove'             => apply_filters( 'Bestwebsite_media_remove_string', _x( 'Remove', 'media', 'meta-box' ) ),
				'edit'               => apply_filters( 'Bestwebsite_media_edit_string', _x( 'Edit', 'media', 'meta-box' ) ),
				'view'               => apply_filters( 'Bestwebsite_media_view_string', _x( 'View', 'media', 'meta-box' ) ),
				'noTitle'            => _x( 'No Title', 'media', 'meta-box' ),
				'loadingUrl'         => admin_url( 'images/spinner.gif' ),
				'extensions'         => self::get_mime_extensions(),
				'select'             => apply_filters( 'Bestwebsite_media_select_string', _x( 'Select Files', 'media', 'meta-box' ) ),
				'or'                 => apply_filters( 'Bestwebsite_media_or_string', _x( 'or', 'media', 'meta-box' ) ),
				'uploadInstructions' => apply_filters( 'Bestwebsite_media_upload_instructions_string', _x( 'Drop files here to upload', 'media', 'meta-box' ) ),
			)
		);
	}
	...
	public static function add_actions() {
		$args  = func_get_args();
		$field = reset( $args );
		add_action( 'print_media_templates', array( Bestwebsite_Helpers_Field::get_class( $field ), 'print_templates' ) );
	}
	...
	public static function value( $new, $old, $post_id, $field ) {
		$new = Bestwebsite_Helpers_Array::from_csv( $new );
	...
	public static function print_templates() {
		require_once Bestwebsite_INC_DIR . 'templates/media.php';
	}
}
