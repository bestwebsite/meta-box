 ...
 */
class Bestwebsite_OSM_Field extends Bestwebsite_Field {
	/**
	...
	public static function admin_enqueue_scripts() {
		...
		wp_enqueue_style( 'Bestwebsite-osm', Bestwebsite_CSS_URL . 'osm.css', array( 'leaflet' ), Bestwebsite_VER );
		wp_enqueue_script( 'Bestwebsite-osm', Bestwebsite_JS_URL . 'osm.js', array( 'jquery', 'jquery-ui-autocomplete', 'leaflet' ), Bestwebsite_VER, true );
	...
	public static function html( $meta, $field ) {
		...
		$html    = sprintf(
			'<div class="Bestwebsite-osm-field" data-address-field="%s">',
			esc_attr( $address )
		);

		$html .= sprintf(
			'<div class="Bestwebsite-osm-canvas" data-default-loc="%s" data-region="%s" data-language="%s"></div>
			<input type="hidden" name="%s" class="Bestwebsite-osm-coordinate" value="%s">',
			esc_attr( $field['std'] ),
	...
	public static function render_map( $location, $args = array() ) {
		...
		wp_enqueue_style( 'leaflet', 'https://unpkg.com/leaflet@1.5.1/dist/leaflet.css', array(), '1.5.1' );
		wp_enqueue_script( 'leaflet', 'https://unpkg.com/leaflet@1.5.1/dist/leaflet.js', array(), '1.5.1', true );
		wp_enqueue_script( 'Bestwebsite-osm-frontend', Bestwebsite_JS_URL . 'osm-frontend.js', array( 'jquery', 'leaflet' ), Bestwebsite_VER, true );
		...
		$output = sprintf(
			'<div class="Bestwebsite-osm-canvas" data-osm_options="%s" style="width:%s;height:%s"></div>',
