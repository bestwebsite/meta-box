 ...
 */
class Bestwebsite_Datetime_Field extends Bestwebsite_Text_Field {
	/**
	...
	public static function admin_register_scripts() {
		$url = Bestwebsite_CSS_URL . 'jqueryui';
		...
		wp_register_style( 'Bestwebsite-date', Bestwebsite_CSS_URL . 'date.css', array( 'jquery-ui-datepicker' ), '1.8.17' );

		wp_register_style( 'jquery-ui-slider', "$url/jquery.ui.slider.css", array( 'jquery-ui-core', 'jquery-ui-theme' ), '1.8.17' );
		wp_register_style( 'jquery-ui-timepicker', "$url/jquery-ui-timepicker-addon.min.css", array( 'Bestwebsite-date', 'jquery-ui-slider' ), '1.5.0' );

		$url = Bestwebsite_JS_URL . 'jqueryui';
		...
		wp_register_script( 'Bestwebsite-datetime', Bestwebsite_JS_URL . 'datetime.js', array( 'jquery-ui-datepicker', 'jquery-ui-timepicker-i18n', 'underscore' ), Bestwebsite_VER, true );
		wp_register_script( 'Bestwebsite-date', Bestwebsite_JS_URL . 'date.js', array( 'jquery-ui-datepicker', 'underscore' ), Bestwebsite_VER, true );
		wp_register_script( 'Bestwebsite-time', Bestwebsite_JS_URL . 'time.js', array( 'jquery-ui-timepicker-i18n' ), Bestwebsite_VER, true );
		...
		foreach ( $handles as $handle ) {
			Bestwebsite_Helpers_Field::localize_script_once( "Bestwebsite-$handle", 'Bestwebsite_' . ucfirst( $handle ), $data );
		}
	}
	...
	public static function admin_enqueue_scripts() {
		...
		wp_enqueue_script( 'Bestwebsite-datetime' );
	}
	...
	public static function html( $meta, $field ) {
		...
		if ( $field['timestamp'] ) {
			...
			$output .= sprintf(
				'<input type="hidden" name="%s" class="Bestwebsite-datetime-timestamp" value="%s">',
		...
		if ( $field['inline'] ) {
			$output .= '<div class="Bestwebsite-datetime-inline"></div>';
		}
	...
	public static function meta( $post_id, $saved, $field ) {
		...
		if ( $field['timestamp'] ) {
			return Bestwebsite_Helpers_Array::map( $meta, __CLASS__ . '::from_timestamp', $field );
		}

		if ( $field['save_format'] && $meta ) {
			return Bestwebsite_Helpers_Array::map( $meta, __CLASS__ . '::from_save_format', $field );
		}
